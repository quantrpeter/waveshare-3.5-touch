...
import lvgl as lv
import builtins
builtins.lv = lv  # the axs15231 touch driver expects `lv` as a global
import task_handler
import i2c
import axs15231
...
_color_index = 0
_SEG_COLOR = lv.color_hex(_COLORS[_color_index])
_all_rects = []
_DIGIT_W = 38
_DIGIT_H = 90
_SEG_T = 10
...
def _make_rect(parent, x, y, w, h):
    ...
    o.set_size(w, h)
    o.set_pos(x, y)
    # Let touches pass through to the screen so swipes are detected there
...
def _apply_color(hex_color):
    color = lv.color_hex(hex_color)
    for rect in _all_rects:
        rect.set_style_bg_color(color, 0)
    date_label.set_style_text_color(color, 0)


def _on_gesture(event):
    global _color_index
    _color_index = (_color_index + 1) % len(_COLORS)
    print("Swipe detected, color -> 0x%06X" % _COLORS[_color_index])
    _apply_color(_COLORS[_color_index])


scrn.add_event_cb(_on_gesture, lv.EVENT.GESTURE, None)
